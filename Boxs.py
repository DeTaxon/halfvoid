...
class BoxMetodFCall:
	...
	if self.IsPoint:
		...
		self.Func = self.ClassType.GetFunc(self.ToCall)
	if self.Func == None:
		raise ValueError("Object does not have that function")
...
class BoxMetod:
	...
	self.PrintFType(F)
        F.write("*")
    def Check(self):
	self.FullName = self.InClass.Name + "." + self.Name 
        PushC()
        for It in self.Params:
            ContextStuff.append(It)
        if self.Block != None:
