ForcedLibs := -f Libs/AVLSet.cp -f Libs/XMLLoader.cp -f Libs/MappedFile.cp  -f Libs/StringBuilder.cp -f Libs/StringSpan.cp -f Libs/WordParser.cp -f Libs/lib.cp -f Libs/Path.cp -f Libs/file.cp -f Libs/arrs.cp -f Libs/FatArray.cp -f Libs/Pair.cp -f Libs/AVLMap.cp -f Libs/MemoryPool.cp -f Libs/HybridQueue.cp -f Libs/Memory.cp -f Libs/Mutex.cp -f Libs/Thread.cp
...
cycle: $(wildcard CompilerReal2/*.cp)
	time ./c.out $(ForcedLibs) CompilerReal2/main.cp -o out3.ll; clang out3.ll $(Libs) -o c.out
repair: $(wildcard CompilerReal2/*.cp) 
	time ./stable $(ForcedLibs) CompilerReal2/main.cp -o out3.ll; clang out3.ll $(Libs) -o c.out
